...
Pull_DockerGS_Image() {
    ...
    if [[ $Pull_DockerGS_Image_process == "y" ]] || [[ $Pull_DockerGS_Image_process == "Y" ]]; then
        ...
        sleep 2s
        Run "sudo docker pull siakbary/dockergs:alpine-gc-3.2" "Pull DockerGS Image" "0" "Menu" "main_menu"
        pathDockerGS=$(sudo docker inspect --format='{{.GraphDriver.Data.LowerDir}}' "siakbary/dockergs:alpine-gc-3.2" | awk '{split($0,a,":"); print a[1]}')
        if [ ! -d "$grasscutter_path" ]; then
            mkdir -p "$grasscutter_path"
        fi
        cd $grasscutter_path || exit 1
        ...
        rm -rf GC*Resources*
        rm resourcesGCAndroid.zip
        Run "sudo docker rmi siakbary/dockergs:alpine-gc-3.2" "Remove DockerGS Image" "0" "Menu" "main_menu"
        echo "${GC}Generate config.json${WC}"
        timeout --foreground 8s java -jar grasscutter.jar &>/dev/null
        echo "${GC}Editing config.json...${WC}"
        if [ ! -f "config.json" ]; then
